...
export async function fetchMeals() {
  const response = await fetch("http://localhost:3000/main/barbecue");
...
export async function fetchLocations() {
  const response = await fetch("http://localhost:3000/main/locations");
...
export async function fetchShopProducts() {
  const response = await fetch("http://localhost:3000/main/shop");
...
export async function postCartData(cartProd: cartProdsData) {
  const response = await fetch("http://localhost:3000/main/shop/add-to-cart", {
    method: "POST",
...
export async function updateQty(updatingProps:{cartItem:cartProdsData,Url:string}) {
  const response = await fetch(
    `http://localhost:3000/main/shop/cart/${updatingProps.Url}-qty`,
    {
      method: "POST",
...
export async function decreaseQty(cartItem: cartProdsData) {
  const response = await fetch(
    "http://localhost:3000/main/shop/cart/decrease-qty",
    {
      method: "POST",
  ...
  if (!response.ok)
    throw new Error("an error occured when decrease qty cart item");
  const n = await response.json();
  return n;
}
